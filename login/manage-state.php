...
if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['action'] === "update-state") {

    try {
        // Validate required fields
        $required_fields = ['editStateId', 'editStateName', 'editStateStatus'];
        ...
        // Sanitize input data
        $editStateId = (int) $_POST['editStateId'];
        $editStateName = trim($_POST['editStateName']);
        $editStateStatus = trim($_POST['editStateStatus']);

        // Check if state record exists
        $checkQuery = "SELECT * FROM state WHERE state_id = ?";
        $checkStmt = $db->prepare($checkQuery);
        $checkStmt->bind_param("i", $editStateId);
        $checkStmt->execute();
        ...
        if ($result->num_rows == 0) {
            echo json_encode([
                "status" => 400,
                "error" => "Invalid  state id required field: "
            ]);
            exit;
        } else {
            // Update existing record
            $updateQuery = "UPDATE state SET 
                state_name = ?, 
                is_active = ?
            WHERE state_id = ?";
...
<body class="">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            ...
                            <ul class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="javascript:void(0);">Mange States</a></li>
                            </ul>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($states as $state) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $state['is_active'] == 1 ? "Active" : "Inactive" ?></td>
                                                <td>
                                                    <div class="dropdown">
                                                        <button class="btn btn-secondary " type="button"
                                                            id="actionMenu<?php echo $state['state_id']; ?>"
                                                            data-bs-toggle="dropdown" aria-expanded="false">
                                                            <i class="feather icon-more-vertical"></i>
                                                        </button>
                                                        <ul class="dropdown-menu"
                                                            aria-labelledby="actionMenu<?= $state['state_id']; ?>">
                                                            <?php if ($isAdmin || hasPermission('Edit State', $privileges, $roleData['role_name'])) { ?>
                                                                <li>
                                                                    <a class="dropdown-item update-state-button"
                                                                        href="javascript:void(0);"
                                                                        data-state-id="<?php echo $state['state_id']; ?>"
                                                                        data-state-name="<?php echo $state['state_name']; ?>"
                                                                        data-state-status="<?php echo $state['is_active']; ?>"
                                                                        data-bs-toggle="modal"
                                                                        data-bs-target="#updateStateModel" title="Edit State">
    ...
    <div class="modal fade" id="updateStateModel" tabindex="-1" aria-labelledby="updateStateLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="updateStateLabel">Update State</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="update-state-form">
                    <div class="modal-body">
                        <input type="hidden" class="form-control" name="editStateId" id="editStateId">
                        <div class="row">
                            <div class="col-6 col-md-6 mb-3">
                                <label for="editReferenceCode" class="form-label">State Name</label>
                            ...
                            <div class="col-6 col-md-6 mb-3">
                                <label for="editStateStatus" class="form-label">Status</label>
                                <div class="input-group">

                                    <select name="editStateStatus" id="editStateStatus" class="form-control">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary editStateButton">Update</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $(document).on("click", ".update-state-button", function (e) {
                e.preventDefault();
                let stateId = $(this).data("state-id");
                let stateName = $(this).data("state-name");
                let stateStatus = $(this).data("state-status");

                $("#editStateId").val(stateId);
                $("#editStateName").val(stateName);
                $("#editStateStatus").val(stateStatus);
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on("submit", ".update-state-form", async function (e) {
                e.preventDefault();

                // Get data from input fields within the form
                let editStateId = $(this).find('input[name="editStateId"]').val().trim();
                let editStateName = $(this).find('input[name="editStateName"]').val().trim();
                let editStateStatus = $(this).find('select[name="editStateStatus"]').val();


                // Basic validation
                if (!editStateId || !editStateName || !editStateStatus) {
                ...
                const $submitBtn = $(this).find('#editStateButton');
                ...
                let formData = {
                    ...
                    editStateStatus: editStateStatus,
                    action: "update-state"
                };
